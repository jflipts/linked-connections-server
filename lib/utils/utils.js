...
module.exports = new class Utils {
        ...
        await Promise.all(datasets.map(async dataset => {
            ...
            if (fs.existsSync(storage + companyName)) {
                ...
                await Promise.all(versions.map(async version => {
                    if (!this._staticFragments[companyName][version]) {
                        this._staticFragments[companyName][version] = {};
                        // Check if there are sub areas defined for this dataset
                        if (dataset.geographicArea.subAreas) {
                            let zones = await readdir(storage + companyName + '/' + version);
                            await Promise.all(zones.map(async zone => {
                                if (!this._staticFragments[companyName][version][zone]) {
                                    let dir = await readdir(storage + companyName + '/' + version + '/' + zone);
                                    let arr = [];
                                    for (let z in dir) {
                                    ...
                                    if (arr.length > 0) {
                                        this._staticFragments[companyName][version][zone] = arr;
                                    }
                                }
                            }));
                        } else {
                            let dir = await readdir(storage + companyName + '/' + version);
                            let arr = [];
                            for (let z in dir) {
                                // Check that this version is not being currently converted
                                if (dir[z].indexOf('.gz') >= 0) {
                            ...
                            if (arr.length > 0) {
                                this._staticFragments[companyName][version] = arr;
                            }
                        }
                    }
                }));
    ...
    findResource(agency, target, versions, zone) {
        let version = null;
        let fragment = null;

        for (let i = 0; i < versions.length; i++) {
            let fragments = this.staticFragments[agency][versions[i]][zone];
    ...
    async addHydraMetada(params) {
        try {
            ...
            jsonld_skeleton['@id'] = host + agency + '/' + zone + '/connections?departureTime=' + departureTime.toISOString();

            let next = this._staticFragments[agency][version][zone][Number(params.index) + 1];
            if (next) {
                jsonld_skeleton['hydra:next'] = host + agency + '/' + zone + '/connections?departureTime=' + new Date(next).toISOString();
            }

            let prev = this._staticFragments[agency][version][zone][Number(params.index) - 1];
            if (prev && prev !== null) {
                jsonld_skeleton['hydra:previous'] = host + agency + '/' + zone + '/connections?departureTime=' + new Date(prev).toISOString();
            }

            jsonld_skeleton['hydra:search']['hydra:template'] = host + agency + '/' + zone + '/connections{?departureTime}';
