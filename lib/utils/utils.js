...
module.exports = new class Utils {
    ...
    /**
     * For every version, return all the pages of a tile
     */
    async getStaticPagesTile(companyName, zoom, x, y) {
        let storage = this.datasetsConfig.storage + '/linked_pages/';
        ...
        // storage/linked_pages/:agency/:version/:zoom/:x/:y
        let versions = await readdir(storage + companyName);
        for (let version of versions) {
            const path = storage + companyName + '/' + version + '/' + zoom + '/' + x + '/' + y;
            ...
            if (fs.existsSync(path)) {
                ...
                // Keep each fragment in milliseconds since epoch in the scan
                for (let z in dir) {
                    // For the internal nodes of the tree, set the time on -1.
                    if (dir[z] == "internal.jsonld.gz") {
                        arr = [-1];
                        break;
                    }
                ...
                if (arr.length > 0) {
                    ...
                    let endDateFile = dir[dir.length - 1];
                    let endDateData = (await this.readAndGunzip(path + '/' + endDateFile)).split(',\n');
                    const dateString = JSON.parse(endDateData[endDateData.length - 1])['departureTime'];
                    pages[version].endDate = new Date(dateString);
                }
            } else {
                throw new Error("Tile /" + zoom + "/" + x + "/" + y + " not found");
            }
        }
        return pages;
    }
    ...
    /**
     *
     * @param {*} versionFragments: for each version the existing pages for a tile
     * @param {*} target: requested timestamp (departureTime)
     * @param {*} sortedVersions: versions sorted by distance to requested version
     *
     * @returns [version, page (startTime), index of page in tile]
     */
    findResourceTile(versionFragments, target, sortedVersions) {
        let version = null;
        let fragment = null;

        for (let i = 0; i < sortedVersions.length; i++) {
            let fragments = versionFragments[sortedVersions[i]];

            // Internal node found
            if (fragments[0] == -1) {
                return [sortedVersions[i], -1, -1];
            }
            // Checking that target date is contained in the list of fragments.
            if (target >= fragments[0] && target <= fragments.endDate.getTime()) {
                fragment = this.binarySearch(target, fragments);
    ...
    binarySearch(target, array) {
        ...
        if (target >= array[min] && (target <= array[max] || target <= array.endDate)) {
            if (array.length === 1) {
                index = 0;
            }
    ...
    async addHydraMetadataTile(params, versionFragments) {
        try {
            let template = await readFile('./statics/skeleton_tiles.jsonld', { encoding: 'utf8' });
            let jsonld_skeleton = JSON.parse(template);

            const { host, agency, departureTime, version, index, data, zoom, x, y} = params;

            const baseUrl = host + agency + '/connections/' + zoom + '/' + x + '/' + y

            jsonld_skeleton['@id'] = baseUrl + '?departureTime=' + departureTime.toISOString();

            jsonld_skeleton['tiles:zoom'] = +zoom;
            jsonld_skeleton['tiles:latitudeTile'] = +y;
            jsonld_skeleton['tiles:longitudeTile'] = +x;

            // Next = next fragment of same tile
            let next = versionFragments[version][Number(index) + 1];
            if (next) {
                jsonld_skeleton['hydra:next'] = baseUrl + '?departureTime=' + new Date(next).toISOString();
            } else {
                delete jsonld_skeleton['hydra:next'];
            }

            // Previous = previous fragment of same tile
            let prev = versionFragments[version][Number(index) - 1];
            if (prev && prev !== null) {
                jsonld_skeleton['hydra:previous'] = baseUrl + '?departureTime=' + new Date(prev).toISOString();
            } else {
                delete jsonld_skeleton['hydra:previous'];
            }
            ...
            // Default search
            jsonld_skeleton['hydra:search'][0]['hydra:template'] = host + agency + '/connections/{zoom}/{x}/{y}{?departureTime}';
            // Location based search
            jsonld_skeleton['hydra:search'][1]['hydra:template'] = host + agency + '/connections/{longitude}/{latitude}{?departureTime}';
    ...
    */
    ...
    getDataset(name) {
        const datasets = this._datasetsConfig.datasets;
        for (let i in datasets) {
