...
module.exports = new class Utils {
    ...
    async getStaticFragmentsTile(companyName, zoom, lat, lon) {
        let storage = this.datasetsConfig.storage + '/linked_pages/';
        ...
        // storage/linked_pages/:agency/:version/:zoom/:lat/:lon
        let versions = await readdir(storage + companyName);
        for (let version of versions) {
            const path = storage + companyName + '/' + version + '/' + zoom + '/' + lat + '/' + lon;
            ...
            if (fs.existsSync(path)) {
                ...
                if (arr.length > 0) {
                    ...
                    fragments[version].endDate = new Date(dateString);
                }
            } else {
                throw new Error("Tile /" + zoom + "/" + lat + "/" + lon + " not found");
            }
        }
        return fragments;
    }
    ...
    async addHydraMetadataTile(params, versionFragments) {
        try {
            ...
            const { host, agency, departureTime, version, index, data, zoom, lat, lon } = params;

            const baseUrl = host + agency + '/connections/' + zoom + '/' + lat + '/' + lon

            jsonld_skeleton['@id'] = baseUrl + '?departureTime=' + departureTime.toISOString();

            jsonld_skeleton['tiles:zoom'] = zoom;
            jsonld_skeleton['tiles:latitudeTile'] = lat;
            jsonld_skeleton['tiles:longitudeTile'] = lon;
