...
class Catalog {
    ...
    async getDistribution(dataset, type) {
        ...
        if (type === 'connections') {
            ...
            if (dataset.zoom) {
                const startDateFilePath = lp_path + '/' + sorted[sorted.length - 1].toISOString() + '/' + dataset.zoom;

                // Loop over all folders of folders to find the start and end dates
                let zoomDir = await readdir(startDateFilePath);
                for (const latDirName of zoomDir) {
                    let latDir = await readdir(startDateFilePath + '/' + latDirName);
                    for (const lonDirName of latDir) {
                        let lonDir = await readdir(startDateFilePath + '/' + latDirName + '/' + lonDirName)
                ...
                const endDateFilePath = lp_path + '/' + sorted[0].toISOString() + '/' + dataset.zoom;

                // Loop over all folders of folders to find the start and end dates
                zoomDir = await readdir(endDateFilePath);
                for (const latDirName of zoomDir) {
                    let latDir = await readdir(endDateFilePath + '/' + latDirName);
                    for (const lonDirName of latDir) {
                        let lonDir = await readdir(endDateFilePath + '/' + latDirName + '/' + lonDirName)
