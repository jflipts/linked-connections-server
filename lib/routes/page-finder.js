...
class PageFinder {
    async getConnections(req, res) {
        ...
        try {
            ...
            // Filter data based on geographical proximity: BOUNDING BOX technique
            ...
            if (locationll != "" && locationur != "") {
                //TODO:
                // stop lat and long should be specified in the stop uri of linked connection
                ...
                logger.debug(locationll_lat)
                logger.debug(locationll_lon)

                // Might have problems when going over the sign border
                jsonld_graph = jsonld_graph.filter(function (el) {
                    return el.arrivalStopLat >= locationll_lat &&
                        el.arrivalStopLat <= locationur_lat &&
                        el.arrivalStopLon >= locationll_lon &&
                        el.arrivalStopLon >= locationur_lon
                })
            }
