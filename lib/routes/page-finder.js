...
class PageFinder {
    async getConnections(req, res) {
        ...
        try {
            ...
            // Filter data based on geographical proximity: BOUNDING BOX technique (ll = lower left corner, ur = upper right corner)
            // Format: geo uri: geo:37.786971,-122.399677
            let locationll = decodeURIComponent(req.query.locationll);
            let locationur = decodeURIComponent(req.query.locationur);

            if (locationll != 'undefined' && locationur != 'undefined') {
                t0 = new Date();

                // TODO Write this url param checker
                if (!this.isValidLocation(locationll) || ! this.isValidLocation(locationur)) {
                    res.set({ 'Cache-Control': 'no-cache' });
                    res.status(404).send("Locations invalid");
                    return;
                }

                const locationll_lat = locationll.split(':')[1].split(',')[0]
                const locationll_lon = locationll.split(':')[1].split(',')[1].split(';')[0]
                const locationur_lat = locationur.split(':')[1].split(',')[0]
                const locationur_lon = locationur.split(':')[1].split(',')[1].split(';')[0]

                let path = storage + '/stops/' + agency + '/stops.json'
                let stops_buffer = (await readfile(path, 'utf8'));
                let stops_data = stops_buffer.split('\n').map(JSON.parse);

                // Build list of stops within bounded box. Works only for good coords for the moment, no negatives
                let stops_in_box = stops_data[0]['@graph'].filter((el) => {
                    return el.latitude >= locationll_lat &&
                    el.latitude <= locationur_lat &&
                    el.longitude >= locationll_lon &&
                    el.longitude >= locationur_lon
                })

                stops_in_box = stops_in_box.map(el => el['@id'])

                jsonld_graph = jsonld_graph.filter(el => {
                    return stops_in_box.includes(el['arrivalStop']) || stops_in_box.includes(el['departureStop'])
                })

                logger.debug('Filtering connections took ' + (new Date().getTime() - t0.getTime()) + ' ms');
            }
    ...
    async getConnectionsTile(req, res) {
        let t0 = new Date();

        // TODO: staticFragments depend per tile so not that usefull to precompute them
        // Check for available updates of the static fragments
        await utils.updateStaticFragments();
        logger.debug('updateStaticFragments() took ' + (new Date().getTime() - t0.getTime()) + ' ms');
        ...
        try {
            // versionFragments is an object containting the different versions for a tile, each version contains a list of fragments and an enddate
            const versionFragments = await utils.getStaticFragmentsTile(agency, req.params.zoom, req.params.lat, req.params.lon)
            const versions = Object.keys(versionFragments)

            // Check if previous version of resource is being requested through memento protocol
            if (req.headers['accept-datetime'] !== undefined) {
                res.set({ 'Cache-Control': 'no-cache' });
                res.status(501).send("Memento not implemented");
                return;
            }
            ...
            // Find the fragment that covers the requested time (static data)
            t0 = new Date();
            let [static_version, found_fragment, index] = utils.findResourceTile(versionFragments, departureTime.getTime(), sorted_versions);
            logger.debug('findResource() took ' + (new Date().getTime() - t0.getTime()) + ' ms');
            let ff = new Date(found_fragment);

            // Redirect client to the appropriate fragment URL
            if (departureTime.getTime() !== found_fragment) {
                res.location('/' + agency + '/connections/' + req.params.zoom + '/' + req.params.lat + '/' + req.params.lon + '?departureTime=' + ff.toISOString());
                res.status(302).send();
                return;
            }

            let sf_path = storage + '/linked_pages/' + agency + '/' + static_version + '/' + req.params.zoom + '/' + req.params.lat + '/' + req.params.lon + '/';
            let rt_exists = false;
            let lowLimit = found_fragment;
            let highLimit = versionFragments[static_version][index + 1];
            ...
            let final_data = await utils.addHydraMetadataTile(params, versionFragments);
