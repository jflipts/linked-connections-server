...
class PageFinder {
    async getConnections(req, res) {
        // Redirect to root
        const dataset = utils.getDataset(req.params.agency)
        if (dataset.tilingOptions && dataset.tilingOptions.supportTree === true) {
            // Allow requests from different hosts
            res.set({ 'Access-Control-Allow-Origin': '*' });

            const rootTile = treeUtils.getRootNode(storage, req.params.agency)
            res.location('/' + req.params.agency + '/connections/' + rootTile.zoom + '/' + rootTile.x + '/' + rootTile.y);
            res.status(302).send();
            return;
        }
    ...
    async getConnectionsTile(req, res) {
        ...
        try {
            ...
            let versionFragments
            try {
                versionFragments = await utils.getStaticPagesTile(agency, req.params.zoom, req.params.x, req.params.y)
            } catch (err) {
                if (err) logger.error(err);
                res.set({ 'Cache-Control': 'no-cache' });
                res.status(404).send("Tile (" + req.params.zoom + "/" + req.params.x + "/" + req.params.y + ") not found");
                return
            }
            ...
            t0 = new Date(); // Timer
            let static_version, found_fragment, index;
            try {
                // Find the fragment that covers the requested time (static data)
                [static_version, found_fragment, index] = utils.findResourceTile(versionFragments, departureTime.getTime(), sorted_versions);
            } catch (err) {
                if (err) logger.error(err);
                res.set({ 'Cache-Control': 'no-cache' });
                res.status(404).send("Page for requested departureTime=" + departureTime.toString() + " not found");
                return;
            }
            logger.debug('findResource() took ' + (new Date().getTime() - t0.getTime()) + ' ms');
            ...
            if (found_fragment == -1) {
                ...
                final_data = await treeUtils.completeInternalNode(params);
                ...
                // Redirect client to the appropriate fragment URL
                if (departureTime.getTime() !== found_fragment) {
                    res.location('/' + agency + '/connections/' + req.params.zoom + '/' + req.params.x + '/' + req.params.y + '?departureTime=' + ff.toISOString());
                    res.status(302).send();
                    return;
                }

                let sf_path = storage + '/linked_pages/' + agency + '/' + static_version + '/' + req.params.zoom + '/' + req.params.x + '/' + req.params.y + '/';
                let rt_exists = false;
                let lowLimit = found_fragment;
                 ...
                 */
                t0 = new Date();
                final_data = await utils.addHydraMetadataTile(params, versionFragments);
                final_data = await treeUtils.addTreeOntologyToPage(params, final_data);
            ...
            logger.debug('Add Metadata took ' + (new Date().getTime() - t0.getTime()) + ' ms');
            logger.debug('Request took ' + (new Date().getTime() - tBegin.getTime()) + ' ms');

        } catch (err) {
            if (err) logger.error(err);
            res.set({ 'Cache-Control': 'no-cache' });
            res.status(404).send("Unspecified Error");
        }
    }
}
