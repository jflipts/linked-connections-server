...
const tiledWriterStream = require('./tiledWriterStream.js');
...
class DatasetManager {
    ...
    initDirs() {
        ...
        if (!fs.existsSync(this.storage + '/tiles')) {
            fs.mkdirSync(this.storage + '/tiles');
        }
    }
    ...
    initCompanyDirs(name) {
        ...
        if (!fs.existsSync(this.storage + '/tiles/' + name)) {
            fs.mkdirSync(this.storage + '/tiles/' + name);
        }
    }
    ...
    async processStaticGTFS(index, dataset) {
        ...
        try {
            ...
            if (file_name != null) {
                ...
                let tBeginStage = new Date().getTime();
                ...
                converter.resultStream(uncompressed_feed, (resultStream, stopsdb) => {
                    ...
                    resultStream.pipe(new Connections2JSONLD(baseURIs, stopsdb))
                        ...
                        .pipe(fileWriter)
                        .on('finish', async () => {
                            let tDurationStage = (new Date().getTime() - tBeginStage) / 1000;
                            logger.info('Stage: Linked Connections Coverter ' + companyName + ' completed successfuly (took ' + tDurationStage + ' seconds)');
                            ...
                            tBeginStage = new Date().getTime();
                            await this.sortLCByDepartureTime(unsorted_path, sorted_path);
                            tDurationStage = (new Date().getTime() - tBeginStage) / 1000;
                            logger.info('Stage: Sort by departure time ' + companyName + ' completed successfuly (took ' + tDurationStage + ' seconds)');
                            ...
                            const reader = fs.createReadStream(sorted_path, 'utf8');
                            const jsonLDStream = reader.pipe(new jsonldstream.Deserializer());

                            tBeginStage = new Date().getTime();
                            if (dataset.tilingStrategy === undefined) {
                                jsonLDStream.pipe(new pageWriterStream(this.storage + '/linked_pages/' + companyName + '/'
                                    + file_name, dataset.fragmentSize || 50000))
                            } else if (dataset.tilingStrategy === "onelevel" || dataset.tilingStrategy === "multilevel") {
                                const path = this.storage + '/linked_pages/' + companyName + '/' + file_name + '/' + dataset.tilingOptions.zoom;
                                // Create folder for Linked Connection fragments
                                fs.mkdirSync(path, { recursive: true });

                                const outputStream = new tiledWriterStream(path, dataset);
                                await outputStream.init();

                                jsonLDStream.pipe(outputStream)
                            }

                            jsonLDStream.on('finish', async () => {
                                tDurationStage = (new Date().getTime() - tBeginStage) / 1000;
                                logger.info('Stage: Pagination and one level fragmentation ' + companyName + ' completed successfuly (took ' + tDurationStage + ' seconds)');

                                if (dataset.tilingStrategy === "multilevel") {
                                    tBeginStage = new Date().getTime();
                                    logger.info(dataset.tilingStrategy + ' tiling ' + companyName + ' Linked Connections fragments...');
                                    const rootpath = this.storage + '/linked_pages/' + companyName + '/' + file_name;
                                    await tileUtils.multilevelTiler(rootpath, dataset, logger);
                                    tDurationStage = (new Date().getTime() - tBeginStage) / 1000;
                                    logger.info('Stage: Multi level fragmentation ' + companyName + ' completed successfuly (took ' + tDurationStage + ' seconds)');
                                }
                                if (dataset.tilingStrategy && dataset.tilingOptions.supportTree === true) {
                                    tBeginStage = new Date().getTime();
                                    logger.info('Building tree ' + companyName + ' Linked Connections fragments...');
                                    const rootpath = this.storage + '/linked_pages/' + companyName + '/' + file_name;
                                    await treeUtils.buildInternalTree(rootpath, dataset, logger);
                                    tDurationStage = (new Date().getTime() - tBeginStage) / 1000;
                                    logger.info('Stage: Building tree ' + companyName + ' completed successfuly (took ' + tDurationStage + ' seconds)');
                                }
                                ...
                                const findcount = child_process.spawnSync('find', ['-type', 'f', '!', '-name', "'*gz'", '-printf', "'.'", '|', 'wc', '-c'], {
                                    cwd: this.storage + '/linked_pages/' + companyName + '/' + file_name,
                                    shell: true
                                })
                                const filesToZip = parseInt(String(findcount.stdout));
                                logger.info('Files to zip: ' + filesToZip);

                                // Zipping a lot of files can take a while
                                let comp = child_process.spawn('gzip', ['-r', '-v', './'], {
                                    cwd: this.storage + '/linked_pages/' + companyName + '/' + file_name,
                                });
                                let filesZipped = 0;
                                let notified = 1;
                                comp.stderr.setEncoding('utf8');
                                comp.stderr.on('data', (error) => {
                                    filesZipped += error.split('\n').length - 1;

                                    if (filesZipped >= notified * filesToZip/10) {
                                        notified++;
                                        logger.info('Files zipped: ' + filesZipped + ' of ' + filesToZip);
                                    }
                                });

                                comp.on('close', async code => {
                                    tDurationStage = (new Date().getTime() - tBeginStage) / 1000;
                                    logger.info('Stage: Compression ' + companyName + ' completed successfuly (took ' + tDurationStage + ' seconds)');
                                    ...
                                    // Update Stops file
                                    let stops = await new Stops().createStopList(companyName);
                                    await writeFile(this.storage + '/stops/' + companyName + '/stops.json', JSON.stringify(stops), 'utf8')
    ...
    async loadGTFSIdentifiers(index, dataset, tmp) {
        try {
            ...
            let real_time_dir = this.storage + '/real_time/' + dataset.companyName;
            ...
            if (lastGtfs !== null) {
                ...
                let [routes, trips, stops, stop_times] = await indexer.getIndexes();
                ...
                this.indexes[index]['stops'] = stops;
                this.indexes[index]['stop_times'] = stop_times;
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                let parser = new Gtfsrt2LC(dataset.realTimeData.downloadUrl, this.indexes[index]['routes'], this.indexes[index]['trips'],
                    this.indexes[index]['stops'], this.indexes[index]['stop_times'], this.getBaseURIs(dataset));
