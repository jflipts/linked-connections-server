                    ...
                    resultStream.pipe(new Connections2JSONLD(baseURIs, stopsdb))
                        ...
                        .on('finish', async () => {
                            ...
                            stops.forEach(stop => {
                                ...
                                stopsMap.set(stop['@id'], [x, y])
                                //stop['longitudeTile'] = tileUtils.long2tile(stop['longitude'], dataset.zoom)
                                //stop['latitudeTile'] = tileUtils.lat2tile(stop['latitude'], dataset.zoom)
                            });

                            // Create folder for Linked Connection fragments
                            fs.mkdirSync(this.storage + '/linked_pages/' + companyName + '/' + file_name + '/' + dataset.zoom, { recursive: true })
                            ...
                            reader.pipe(new jsonldstream.Deserializer())
                                // TODO, rework so this also works for not tiled configs
                                //.pipe(new pageWriterStream(this.storage + '/linked_pages/' + companyName + '/'
                                    //+ file_name, dataset.fragmentSize || 50000))
                                .pipe(new tiledWriterStream(this.storage + '/linked_pages/' + companyName + '/' + file_name  + '/' + dataset.zoom,
                                dataset.fragmentSize || 50000, stopsMap))
                                .on('finish', async () => {
                                    ...
                                    comp.on('close', async code => {
                                        ...
                                        // await writeFile(this.storage + '/linked_pages/' + companyName + '/' + file_name  + '/' + dataset.zoom + '/tiles.json', JSON.stringify(tiles), 'utf8')
